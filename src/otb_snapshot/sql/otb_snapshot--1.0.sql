...
BEGIN
    ...
    -- 检查快照是否已存在
    IF EXISTS (
        SELECT 1 FROM otb_snapshot.snapshots 
...
BEGIN
    ...
    -- 删除快照记录
    DELETE FROM otb_snapshot.snapshots
    WHERE table_name = p_table_name AND snapshot_name = p_snapshot_name;
