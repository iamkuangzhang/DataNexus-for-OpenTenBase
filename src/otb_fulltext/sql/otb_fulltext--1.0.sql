...
BEGIN
    ...
    -- 遍历字节，识别UTF-8编码的中文字符（3字节）
    i := 1;
    WHILE i <= octet_length(input_text) LOOP
        byte_val := get_byte(input_text::bytea, i - 1);
        
        IF byte_val >= 224 AND byte_val <= 239 THEN
            -- UTF-8 中文字符起始字节（3字节序列）
            IF i + 2 <= octet_length(input_text) THEN
                current_char := convert_from(
                    substring(input_text::bytea from i for 3),
                    'UTF8'
                );
...
DECLARE
    ...
    dict_id INTEGER;
BEGIN
    SELECT id INTO dict_id FROM otb_fulltext.dictionaries WHERE name = dict_name;
    IF dict_id IS NULL THEN
        RETURN tokens;  -- 字典不存在，返回原数组
    END IF;
    
    FOREACH t IN ARRAY tokens LOOP
        IF NOT EXISTS(SELECT 1 FROM otb_fulltext.stopwords WHERE dict_id = dict_id AND word = t) THEN
